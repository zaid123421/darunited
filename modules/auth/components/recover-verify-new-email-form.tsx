...
export function RecoverVerifyNewEmailForm() {
  const { verifyNewPrimaryEmail } = useAuth();
  const [digits, setDigits] = useState<string[]>(Array(OTP_LENGTH).fill(""));
  const inputsRef = useRef<Array<HTMLInputElement | null>>([]);

  const code = digits.join("");
  const isRateLimited =
    verifyNewPrimaryEmail.error instanceof ApiError &&
    verifyNewPrimaryEmail.error.statusCode === 429;
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    verifyNewPrimaryEmail.mutate(code);
  };

  return (
    <div>
      <Link
        href="/recover/new-email"
      ...
      <div className="mb-7 sm:mb-8">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          Enter the 6-digit code sent to your new primary email. You&apos;ll
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        {verifyNewPrimaryEmail.error ? (
          ...
          >
            {verifyNewPrimaryEmail.error.message}
          </div>
        ) : null}
        ...
        <button
          type="submit"
          disabled={
            verifyNewPrimaryEmail.isPending || code.length !== OTP_LENGTH
          }
        ...
        >
          {verifyNewPrimaryEmail.isPending ? (
            <span className="flex items-center gap-2">
              <span className="h-4 w-4 animate-spin rounded-full border-2 border-primary/30 border-t-primary" />
              Confirming…
            </span>
          ) : (
            "Confirm & go to sign in"
          )}
        </button>
      </form>
    </div>
